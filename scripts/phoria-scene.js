...
(function() {
   ...
   {
      // merge structures to generate scene
   ...
   Phoria.Scene.prototype = {
      // camera - converts values to vec3 to generate camera matrix
      camera: null,
      
      // the near/far values are distances from the camera view plane, and are always positive.
      // the perspective frustrum moves with the viewer
      perspective: null,
      
      // manipulate 3D entity graph directly e.g. push/delete objects
      graph: null,

      // dimensions of viewport for NDC->viewport conversion step
      viewport: null,

      // the flattened, sorted list of entities for rendering a frame of the scene - updated by modelView()
      renderlist: null,

      // the light entities that were found when processing the scene graph - updated by modelView()
      lights: null,
            ...
            for (var n=0, obj, len, isIdentity; n<entities.length; n++)
            {
               ...
               else if (obj instanceof Phoria.Entity)
               {
                  ...
                  if (objClip !== len)
                  {
                     ...
                     {
                        // NOTE: could make this simplier - have a flag on scene for "transposedNormalMatrix..."?
